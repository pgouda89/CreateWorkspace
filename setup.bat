...
IF EXIST "%USERPROFILE%\Desktop\%RELEASE%\%RELEASE%_%P4CLIENT%.bat" ECHO Successfully created the Desktop shortcut : %USERPROFILE%\Desktop\%RELEASE%\%RELEASE%_%P4CLIENT%.bat
REM Done
...
IF "%SKIP_DESKTOP_SHORTCUT%"=="TRUE" EXIT /B
IF NOT EXIST "%USERPROFILE%\Desktop\%~1" (
MKDIR "%USERPROFILE%\Desktop\%~1"
)
IF EXIST "%USERPROFILE%\Desktop\%~1\%~1_%~2.bat" del %USERPROFILE%\Desktop\%~1\%~1_%~2.bat

more %envFile% > %USERPROFILE%\Desktop\%~1\%~1_%~2.bat
echo CALL %~sdp0\scripts\envFile_default.bat >> %USERPROFILE%\Desktop\%~1\%~1_%~2.bat
echo cmd /k >> %USERPROFILE%\Desktop\%~1\%~1_%~2.bat
